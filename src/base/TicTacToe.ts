...
class TicTacToe extends EventEmitter implements GameOptions {
    ...
    private flattenGrid(grid: any[]): (string | number)[] {
    ...
    public get flatGrid(): (string | number)[] {
    ...
    public start(): void {
        ...
        if (!firstPlayer) throw new Error('Something wrong with the firstPlayer');
        const secondPlayer = this.data.players.find(player => player.symbol === "o");
        if (!secondPlayer) throw new Error('Something wrong with the secondPlayer');
        this.data.player1 = firstPlayer.id;
        this.data.player2 = secondPlayer.id;
        this.data.currentPlayer = firstPlayer.id;
        if (this.bot && firstPlayer.id === 42) this.botMove();
        if (this.bot && this.training && firstPlayer.id === 666) this.trainingMove();
