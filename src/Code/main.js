...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeGame() {
        ...
        if (morpion.data.player1 === 42) {
            ...
            updateGameInfo("First player : you");
        }
    }
    async function removeBoard() {
        await document.querySelectorAll("div.cell").forEach((c) => c.remove())
    ...
    function handleCellClick(event) {
        if (!data || !morpion.data.started || morpion.data.ended) return;
        const cell = event.target;
        const row = cell.dataset.row;
        const col = cell.dataset.col;

        try {
            morpion.player.move(parseInt(col), parseInt(row))
            cell.textContent = morpion.data.players.find(p => p.id === 666).symbol;
            cell.removeEventListener('click', handleCellClick);
            if (morpion.data.ended) {
                endGame();
                updateGameBoard();
                return;
            }
            updateGameBoard();
            setTimeout(() => {
                if (morpion.data.ended) return endGame();
                morpion.botMove();
                updateGameBoard();
                if (morpion.data.ended) {
                    endGame();
    ...
    function updateGameBoard() {
        if (!data || !morpion.data.started || morpion.data.ended) return;
        const currentBoard = morpion.grid;
        currentBoard.forEach((row, i) => {
            row.forEach((cell, j) => {
                const cellDiv = gameboardElement.children[i * gridSize + j];
                if (cell[0] === 0) {
                    cellDiv.textContent = "";
    ...
    function endGame() {
        if (data && !morpion.data.ended) {
            morpion.data.ended = true;
        }
        if (morpion.data.winner === 42) {
            updateGameInfo("and against all odds you loose..")
        } else if (morpion.data.winner === 666) {
            updateGameInfo("congrats you won !")
        } else {
            updateGameInfo("another draw..")
        }
        removeBoard()
        data = false;
    }
