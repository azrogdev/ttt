import { GameInstance, TicTacToe, Player } from "../main.js";
...
function validateProperty(value: any, expectedTypes: string | string[], propertyName: string): void {
...
function Save(instance: TicTacToe, toJSON: boolean = false): (GameBackup | string |undefined) {
    if (!(instance instanceof TicTacToe)) throw new Error(`Invalid 'instance' received.`);
    validateProperty(toJSON, "boolean", "toJSON");
    if (!instance.data || !instance.data.started || instance.data.ended) return undefined;
...
function Restore(backup: GameBackup | string): TicTacToe {
    var backupData: GameBackup;
    ...
    if (!Array.isArray(flatGrid) || flatGrid.length !== 9) throw new Error(`Invalid 'flatGrid' received.`);
    ...
    if (!Array.isArray(data.players) || data.players.length !== 2) throw new Error(`Invalid 'data.players' received.`)
    ...
    if (data.winner !== null) validateProperty(data.winner, "object", "data.winner");
